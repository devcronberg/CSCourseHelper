...
namespace MCronberg.PeopleHelper
{
        ...
        public Person GetPerson(int PersonId)
        {
            Person p = new Person();
            {
                ...
                if(p.Gender== Gender.Male)
                    p.Name = PersonHelperMethods.GetRandomMaleName();
                else
                    p.Name = PersonHelperMethods.GetRandomFemaleName();
                p.DateOfBirth = DateTime.Now.Date.AddDays(-r.Next(1, 36000));
                p.Height = r.Next(150, 200);
                p.IsHealthy = r.Next(0, 10) >= 2 ? true : false;
